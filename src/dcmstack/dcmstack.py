...
with warnings.catch_warnings():
    ...
    from .extract import ExtractedDcmWrapper, ExtractedDcmMosaicWrapper
...
class DicomStack(object):
    ...
    def to_nifti(self, voxel_order='rpi', embed_meta=False):
        ...
        self._ismosaic = self._files_info[0][0].is_mosaic
...
def parse_and_stack(src_paths, key_format='%(SeriesNumber)03d-%(ProtocolName)s', 
    ...
    for dcm_path in src_paths:
        try:
            ...
            ismosaic = 'MOSAIC' in dcm.ImageType 
            if not ismosaic:
                wrp_dcm = ExtractedDcmWrapper.from_dicom(dcm, extractor)
            else:
                wrp_dcm = ExtractedDcmMosaicWrapper.from_dicom(dcm, extractor)
